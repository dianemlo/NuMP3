...
// Keep this focused on truly bad/undesired variants.
// Do NOT globally reject remasters / mono / stereo mix / edit / version.
// Those are often the correct official catalog uploads.
const BAD_TITLE =
  /\b(official\s*(music\s*)?video|video\s*version|vevo|\bmv\b|promo|visualizer|live|performance|session|sessions|siriusxm|kexp|npr|tiny\s*desk|cover|acoustic|reaction|interview|trailer|karaoke|sped\s*up|slowed|remix|full\s*album|playlist|demo|instrumental|fan\s*made|tribute)\b/i;
...
function looksRelevantToTrack(item, artist, title) {
  ...
  // Reject only obvious bad variants here.
...
async function findViaArtistTopicChannel(artist, title) {
  ...
  for (const item of items) {
    ...
    if (BAD_TITLE.test(itemTitleRaw)) continue;
...
async function findYouTubeCandidatesForTrack(
  { artist, title },
  {
    maxResultsPerQuery = 5,
    preferOfficialAudio = true,
  ...
  function pushCandidate(item, bonus = 0) {
    if (!item?.videoId) return;
    if (seen.has(item.videoId)) return;
    if (BAD_TITLE.test(item.title || "")) return;
    if (!looksRelevantToTrack(item, a, t)) return;

    seen.add(item.videoId);

    ranked.push({
      ...item,
      _score: scoreYT(item, { preferOfficialAudio }) + bonus,
    });
  }

  const topicMatch = await findViaArtistTopicChannel(a, t).catch(() => null);
  if (topicMatch?.videoId) {
    pushCandidate(topicMatch, 20);
  }
  ...
  for (const q of queries) {
    const items = await youtubeSearch(q, maxResultsPerQuery).catch(() => []);

    for (const item of items || []) {
      if (!item?.videoId) continue;

      let bonus = 0;
      if (isStrongOfficialMatch(item)) bonus += 12;

      pushCandidate(item, bonus);
    }
  }
  ...
  for (const item of ranked) {
    ...
    try {
      verified = await verifyCandidateOwnership(item, a);
    } catch {
      verified = false;
    }
    ...
    const acceptable =
      verified ||
      (allowNonOfficialFallback &&
        (
          /\s*-\s*topic\s*$/i.test(item?.channel || "") ||
          /provided\s+to\s+youtube/i.test(item?.title || "") ||
          looksLikeArtistOwnedOrOfficial(item, a)
        ));
  ...
  filtered.sort((x, y) => y._score - x._score);

  return filtered.slice(0, limit).map(({ _score, ...item }) => item);
}
