...
function Profile() {
  ...
  async function loadSavedAlbums(token) {
    try {
      const res = await fetch('http://localhost:5000/api/users/albums', {
  ...
  async function handleRemoveAlbum(albumId) {
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`http://localhost:5000/api/users/albums/${albumId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      const data = await res.json()
              ...
              savedAlbums.map((album) => (
                ...
                >
                  ...
                  <div style={{ fontSize: "0.75rem", opacity: 0.6 }}>
                    {album.release_date ? new Date(album.release_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''}
                  </div>
